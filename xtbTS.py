...
os.system("cp " + xyzFile + " scan/")
f=open("scan/xcontrol","w+")
command = "$constrain \n  force constant = 1 \n  distance: " + str(atomNo1) + "," + str(atomNo2) + "," + str(startScanDistance) + "\n$scan\n  1: " + str(startScanDistance) + "," + str(finalScanDistance) + "," + str(steps) + " \n$end\n"
f.write(command)
f.close()
os.chdir("./scan")
os.system("xtb " + xyzFile + params + "--opt veryfine --input xcontrol | tee opt.out")
...
for i in range(0,steps):
	energy = float(f1[i*(totalAtoms+2)+1].split()[-1])*627.509 - energy0
...
os.system("cp TSguess.xyz sp_ohess/")
os.chdir("./sp_ohess")
f=open("xcontrol","w+")
