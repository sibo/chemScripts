...
def PESscan(direction, xyzFile, atomNo1, atomNo2, finalScanDistance, startScanDistance, steps):
	os.system("mkdir -p scan_" + direction)
	os.system("cp " + xyzFile + " scan_" + direction + "/")
	f=open("scan_" + direction + "/xcontrol","w+")
	command = "$constrain \n  force constant = 1 \n  distance: " + str(atomNo1) + "," + str(atomNo2) + "," + str(startScanDistance) + "\n$scan\n  1: " + str(startScanDistance) + "," + str(finalScanDistance) + "," + str(steps) + " \n$end\n"
	f.write(command)
	f.close()
	os.chdir("./scan_" + direction)
	...
	f.close()
	os.chdir("../")
	return maxEnergy,maxStruct

(maxEnergy1,maxStruct1)=PESscan("forward", xyzFile, atomNo1, atomNo2, finalScanDistance, startScanDistance,steps)
(maxEnergy2,maxStruct2)=PESscan("backward","scan_forward/xtbopt.xyz", atomNo1, atomNo2, startScanDistance, finalScanDistance,steps)
